// game.js — Phaser (ES5-safe) with multi-start failsafe for mobile
(function () {
  'use strict';
  ...
  // ---------- START FAILSAFE (works on tap/click anywhere on overlay) ----------
  var titleEl = document.getElementById('title');
  var startBtn = document.getElementById('startBtn');
  var bootArmed = false, booted = false;
  ...
  function boot(){
    ...
    // remove starters so they don't fire later
    disarmStart();
  }

  function onStartEvent(e){ if(e&&e.preventDefault) e.preventDefault(); boot(); }

  function armStart(){
    if (bootArmed) return; bootArmed = true;
    // Button
    if (startBtn){
      startBtn.addEventListener('click', onStartEvent, {passive:true});
      startBtn.addEventListener('touchstart', onStartEvent, {passive:false});
      startBtn.addEventListener('pointerdown', onStartEvent, {passive:false});
    }
    // Whole overlay (tap anywhere)
    if (titleEl){
      titleEl.addEventListener('click', onStartEvent, {passive:true});
      titleEl.addEventListener('touchstart', onStartEvent, {passive:false});
      titleEl.addEventListener('pointerdown', onStartEvent, {passive:false});
      // Make sure the overlay can receive taps
  ...
  function disarmStart(){
    if (!bootArmed) return; bootArmed = false;
    function safeRm(el, type, fn){ try{ el && el.removeEventListener(type, fn); }catch(e){} }
    if (startBtn){
      safeRm(startBtn,'click',onStartEvent);
      safeRm(startBtn,'touchstart',onStartEvent);
      safeRm(startBtn,'pointerdown',onStartEvent);
    }
    if (titleEl){
      safeRm(titleEl,'click',onStartEvent);
      safeRm(titleEl,'touchstart',onStartEvent);
      safeRm(titleEl,'pointerdown',onStartEvent);
    }
  }

  // Arm as soon as this file loads
  armStart();
  ...
  function create(){
    ...
    // Scale big frames down to ~128px tall (384 -> 128)
    var targetH = 128, scale = targetH / 384;
